defmodule LiveViewPhoenixWeb.AutocompleteLive do
  use LiveViewPhoenixWeb, :live_view

  alias LiveViewPhoenix.Stores
  alias LiveViewPhoenix.Cities
  ...
  def handle_info({:run_city_search, city}, socket) do
    case Stores.search_by_city(city) do
      [] ->
        socket =
          socket
          |> put_flash(:info, "No stores matching\"#{city}\"") #shows a message on the screen when it does not find the postal code
          |> assign(stores: [], loading: false)
